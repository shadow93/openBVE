						...
						if (SoundSources[i] != null && !SoundSources[i].FinishedPlaying) {
							...
							if (distanceSquared < outerRadiusSquared) {
								...
								if (startPlaying || IsPlaying(i)) {
									...
									int j = SoundSources[i].OpenAlSourceIndex.Index;
									AL.Source(j, ALSource3f.Position, SoundSources[i].OpenAlPosition[0],
										SoundSources[i].OpenAlPosition[1],
										SoundSources[i].OpenAlPosition[2]);
									AL.Source(j, ALSource3f.Velocity, SoundSources[i].OpenAlVelocity[0],
										SoundSources[i].OpenAlVelocity[1],
										SoundSources[i].OpenAlVelocity[2]);
